...
for bagname in BAGNAMES:
    ...
    for tod in [TOD.OFF]:
        ...
        pval_dangerous_interaction2.append([METRICS[i]['human_collision'] for i in range(90)])
        for task in tqdm(list(np.arange(NTASK)), desc=f"{bagname}-{tod.value}"):
            pval_travelled_distance[bagname]["Planned Travelled Distance"].append(METRICS[task]['travelled_distance_planned'])
            if METRICS[task]['result'] == 1:
                pval_working_time[bagname]["Active Time"].append(METRICS[task]['time_to_reach_goal_actual'])
                pval_working_time[bagname]["Stalled Time"].append(METRICS[task]['time_to_reach_goal_stalled'])
                pval_travelled_distance[bagname]["Extra Travelled Distance"].append(METRICS[task]['travelled_distance_actual'] - METRICS[task]['travelled_distance_planned'])
                pval_battery[bagname]["Effective Battery Usage"].append(METRICS[task]['battery_consumption_actual'])
            else:
                pval_working_time[bagname]["Wasted Time"].append(METRICS[task]['time_to_reach_goal_wasted'])
                pval_travelled_distance[bagname]["Wasted Travelled Distance"].append(METRICS[task]['travelled_distance_wasted'])
                pval_battery[bagname]["Wasted Battery Usage"].append(METRICS[task]['battery_consumption_wasted'])
...
for metric_type, metric_list in metrics.items():
    ...
    if metric_type == "pval_success_failure":
        ...
        overall_contingency_table = [
            pval_success_failure[0][:-1],  # Baseline row (Success, Failure-People, Failure-Battery)
            pval_success_failure[1][:-1]   # Causal row
        ]
        test_used = "Chi-Square"
...
pvalues_path = os.path.join(OUTDIR, "pvalues-off.pkl")
