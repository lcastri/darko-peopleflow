...
class Scheduler:
    def __init__(self, params,action_nodes,trays,objects,rewards,action_times):
        ...
        self.V = _backward_pass(self.V,self.n_action_nodes,states,self.coeff,self.qft,self.quantities,
                                self.n_objects,self.n_trays,self.picking_reward,self.placing_reward,self.picking_time,self.throwing_time,
                                self.navigation_risk_mtx,self.picking_prob_mtx,self.throwing_prob_mtx,self.alpha_risk,self.t_horizon,self.o_max,self.wait_time,self.drop_time)
    ...
    def available_actions(self,state):
        p1,r1s,s1s,r1f,s1f = _available_actions(state,self.t_horizon,self.o_max,self.n_action_nodes,self.n_objects,self.n_trays,
                                                     ...
                                                     self.alpha_risk,self.wait_time,self.drop_time)
    ...
    def next_task(self,state):
        ...
        typ,new_state,next_state = _forward_step(state,self.V,self.coeff,self.qfa,self.t_horizon,self.o_max,self.n_action_nodes,self.n_objects,self.n_trays,self.picking_reward,
                                                 self.placing_reward,self.picking_time,self.throwing_time,self.quantities,self.navigation_risk_mtx,
                                                 self.picking_prob_mtx,self.throwing_prob_mtx,self.alpha_risk,self.wait_time,self.drop_time)
...
@nb.njit
def _available_actions(state,t_horizon,max_objects,n_action_nodes,n_objects,n_trays,picking_reward,placing_reward,picking_time,throwing_time,quantities,navigation_risk_mtx,picking_prob_mtx,throwing_prob_mtx,alpha_risk,wait_time,drop_time):
    ...
    if (obj_on_tray<max_objs):
        for o in range(n_objects):
            ...
            cond_3 = picking_prob_mtx[n0,o] > 50 # controllo che la probabilità di fare picking sia non nulla
            if cond_1 and cond_2 and cond_3:
                p1_ay[ns] = picking_prob_mtx[n0,o]
                # success state
                r1_A_ay[ns] = picking_reward*0
                s1_A_mtx[ns, :] = state[:]  
    ...
    if (obj_on_tray > 0):
        for o in range(n_objects):
            for t in range(n_trays):
                ...
                cond_4 = throwing_prob_mtx[n0,t] > 50 # controllo che la probabilità di lanciare nel vassio t sia non nulla
                ...
                if (cond_1 and cond_2 and cond_3 and cond_4):
                    ...
                    ns += 1  
    # Waiting actions
    _,risk = max(1,navigation_risk_mtx[n0,n,1]), navigation_risk_mtx[n0,n,3]
    cond= (state[0]+wait_time) < t_horiz
    if cond:
        p1_ay[ns]=100
        #success state
        r1_A_ay[ns] = 0#-alpha_risk*risk  
        s1_A_mtx[ns, :] = state[:]
    ...
    if (obj_on_tray>0):
        for o in range(n_objects):
            cond_1 = (state[0] + drop_time) < t_horiz # controllo di essere dentro l'orizzonte temporale alla fine dell'azione di throwing
            cond_2 = state[(n_trays+1) * o + 2] > sum([state[(n_trays+1) * o + 3 + t] for t in range(n_trays)]) # controllo di avere almeno un oggetto di tipo o sul vassio
            if (cond_1 and cond_2):
                p1_ay[ns] = 100
                # success state
                r1_A_ay[ns] = 0#-picking_reward
                s1_A_mtx[ns, :] = state[:]
...
def _backward_pass(V,n_action_nodes,states,coeff,qft,quantities,n_objects,n_trays,picking_reward,placing_reward,picking_time,throwing_time,
                   navigation_risk_mtx,picking_prob_mtx,throwing_prob_mtx,alpha_risk,t_horizon,o_max,wait_time,drop_time):
            ...
            for q in states:
                ...
                if q == qft:
                    ...
                    p1,r1s,s1s,r1f,s1f = _available_actions(s0,t_horizon,o_max,n_action_nodes,n_objects,n_trays,
                                                            ...
                                                            quantities,navigation_risk_mtx,picking_prob_mtx,throwing_prob_mtx,alpha_risk,wait_time,drop_time)
                    if (len(p1) == 0):
                        V[s0] = _final_state_value(s0, quantities, t_horizon, n_objects, n_trays)
                    else:
...
@nb.njit
def _forward_step(state,V,coeff,qfa,t_horizon,o_max,n_action_nodes,n_objects,n_trays,picking_reward,placing_reward,picking_time,throwing_time,quantities,navigation_risk_mtx,picking_prob_mtx,throwing_prob_mtx,alpha_risk,wait_time,drop_time):
    #     # 0 -> end
    #     # 1 -> pick/place/wait/drop
    #     # 2 -> move
    ...
    while True:
        ...
        p1,r1s,s1s,r1f,s1f = _available_actions(s_move,t_horizon,o_max,n_action_nodes,n_objects,n_trays,picking_reward,placing_reward,picking_time,throwing_time,quantities,navigation_risk_mtx,picking_prob_mtx,throwing_prob_mtx,alpha_risk,wait_time,drop_time)
